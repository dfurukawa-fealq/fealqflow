    ...
    <section class="bg-surface-container-low border border-outline-variant rounded shadow-sm p-lg">
      ...
      <div class="flex flex-col gap-sm">
        ${programa.documentos.map(doc => {
          ...
          if (isGerado) {
            actionHtml = '<span class="text-[#F17C58] text-[11px] font-bold uppercase shrink-0">Pendente</span>';
            iconHtml = '<span class="material-symbols-outlined text-[18px] shrink-0 text-outline doc-icon">radio_button_unchecked</span>';
          } else if (isExterno) {
            actionHtml = '<span class="text-on-surface-variant text-[11px] font-bold uppercase shrink-0">Externo</span>';
            iconHtml = '<span class="material-symbols-outlined text-[18px] shrink-0 text-outline doc-icon">forward_to_inbox</span>';
          } else {
            iconHtml = '<span class="material-symbols-outlined text-[18px] shrink-0 text-outline doc-icon">radio_button_unchecked</span>';
            actionHtml = `
              <div class="flex items-center gap-sm shrink-0">
                ${doc.url_template ? `<a href="${doc.url_template}" target="_blank" class="text-primary-container text-[11px] font-bold uppercase hover:underline shrink-0">Modelo</a>` : ''}
                <div class="relative cursor-pointer">
                  <input type="file" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer doc-upload" data-id="${doc.id}">
                  <span class="text-secondary text-[11px] font-bold uppercase hover:underline">Anexar</span>
                </div>
              </div>
            `;
          }
          ...
          return `
            <div data-tipo="${doc.tipo}" class="doc-card p-sm bg-surface-container-lowest border rounded transition-colors ${isGerado ? 'border-outline-variant doc-card concluido' : 'border-outline-variant border-dashed doc-card'}">
              ...
              <div class="doc-file-info hidden mt-2 flex items-center justify-between px-3 py-2 bg-surface-container-low border border-outline-variant rounded">
                <div class="flex items-center gap-2 min-w-0">
                  <span class="material-symbols-outlined text-[16px] text-on-surface-variant">description</span>
                  <span class="text-[11px] font-bold text-on-surface truncate doc-filename">arquivo.pdf</span>
                </div>
                <button type="button" class="doc-remove text-on-surface-variant hover:text-[#F17C58] transition-colors material-symbols-outlined text-[16px]">close</button>
              </div>
              ${detalhe ? `<p class="text-[10px] text-on-surface-variant mt-xs leading-tight pl-[26px]">${detalhe}</p>` : ''}
              ${doc.acaoEmail ? `
                <a href="mailto:${doc.acaoEmail.email}" class="inline-flex items-center gap-1 mt-xs ml-[26px] text-[11px] font-medium text-primary hover:text-primary-container transition-colors">
                  <span class="material-symbols-outlined text-[14px]">mail</span>
                  ${doc.acaoEmail.label}
                </a>
              ` : ''}
              ${!doc.obrigatorio ? `
                <span class="inline-block mt-xs ml-[26px] text-[9px] font-bold uppercase tracking-wide px-1.5 py-[1px] rounded bg-surface-variant text-on-surface-variant">
                  Condicional
                </span>
              ` : ''}
            </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      uploads.forEach(input => {
        input.addEventListener('change', (e) => {
          ...
          const filename = card.querySelector('.doc-filename');
